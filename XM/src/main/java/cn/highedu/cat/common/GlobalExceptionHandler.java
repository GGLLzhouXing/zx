...
public class GlobalExceptionHandler {
   ...
   // }
    ...
    public JsonResult<Void> handleResponseStatusException(ResponseStatusException e) {
        log.warn("程序运行过程中出现ResponseStatusException，将统一处理！");
        log.warn("异常信息：{}", e.getMessage());
        HttpStatus status = e.getStatus();
        if (status == HttpStatus.NOT_FOUND) {
            return JsonResult.fail(ServiceCode.ERR_NOT_FOUND, e.getReason());
        } else if (status == HttpStatus.FORBIDDEN) {
            return JsonResult.fail(ServiceCode.ERR_UNAUTHORIZED, e.getReason());
        } else if (status == HttpStatus.BAD_REQUEST) {
            return JsonResult.fail(ServiceCode.ERR_BAD_REQUEST, e.getReason());
        }
